# Attribution: project-status-report

Original skill: `status-report`.
Original author: Anthropic and upstream contributors
Source: https://github.com/anthropics/knowledge-work-plugins/blob/1c7187c4fc17feefa6cde39517f12dae1249e6c4/operations/skills/status-report/SKILL.md
...
- `SKILL.md` from `anthropics/knowledge-work-plugins/operations/skills/status-report/SKILL.md`: Relocate original connector-category reference inside this package.; Bind existing sibling guidance to local instructions, without claiming a separate installed dispatcher.; Use the clear installed name, standard licence and string-valued attribution metadata; preserve all other upstream fields and body. Normalize equivalent tool-list separators where required.; Identify adapted files while retaining upstream authorship and licence.
- `LICENSE` from `anthropics/knowledge-work-plugins/LICENSE`: Copied unchanged.
- `references/CONNECTORS.md` from `anthropics/knowledge-work-plugins/operations/CONNECTORS.md`: The original native plugin configuration is optional upstream setup documentation, not an installed local manifest.; Identify adapted files while retaining upstream authorship and licence.
- `references/risk-assessment.md` from `anthropics/knowledge-work-plugins/operations/skills/risk-assessment/SKILL.md`: Copied unchanged.

## Runtime requirements

- Project/team reporting period,actual status,KPIs and action owners supplied directly or through optional tracker/chat/calendar connectors.
- Posting to chat is optional authored functionality and requires available connector capabilities.
