# Attribution: employment-offer-drafting

Original skill: `draft-offer`.
Original author: Anthropic and upstream contributors
Source: https://github.com/anthropics/knowledge-work-plugins/blob/1c7187c4fc17feefa6cde39517f12dae1249e6c4/human-resources/skills/draft-offer/SKILL.md
...
- `SKILL.md` from `anthropics/knowledge-work-plugins/human-resources/skills/draft-offer/SKILL.md`: Relocate original connector-category reference inside this package.; Use the clear installed name, standard licence and string-valued attribution metadata; preserve all other upstream fields and body. Normalize equivalent tool-list separators where required.; Identify adapted files while retaining upstream authorship and licence.
- `LICENSE` from `anthropics/knowledge-work-plugins/LICENSE`: Copied unchanged.
- `references/CONNECTORS.md` from `anthropics/knowledge-work-plugins/human-resources/CONNECTORS.md`: The original native plugin configuration is optional upstream setup documentation, not an installed local manifest.; Identify adapted files while retaining upstream authorship and licence.

## Runtime requirements

- Actual compensation, employment classification, benefits, start date and manager from the company
- human-resources/CONNECTORS.md; optional HRIS for bands/headcount and optional ATS for candidate/status
- If ATS is connected, its native workflow may update offer status; connector permissions must be supported
